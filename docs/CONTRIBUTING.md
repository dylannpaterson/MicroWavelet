...
Maintainers should release from `main` with the manual GitHub Actions release workflow. The workflow bumps the version, reserves a Zenodo DOI when credentials are configured, tags the release, creates GitHub release notes from commits since the previous tag, and publishes to PyPI when PyPI credentials are configured.
