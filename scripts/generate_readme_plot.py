...
def generate_plot():
    ...
    ax1.plot(
        ...
        linestyle="--",
        alpha=0.4,
        label="Local Gaussian Interpolation",
        linewidth=1.2,
    )
    ...
    if results["anomalies"]:
        ...
        ax1.plot(
            ...
            fit_A,
            color="#2ecc71",
            label=f"Best-Fit Paczynski ($t_E={anom['tE']:.2f}$ d)",
            linewidth=2.0,
        )
        ax1.axvline(
            anom["t0"],
            ...
            linestyle=":",
            alpha=0.8,
            label=f"Peak Centre $t_0={anom['t0']:.2f}$",
        )

    ax1.set_ylabel("Relative Flux", fontsize=11, fontweight="bold")
    ax1.set_title(
        "MicroWavelet: CWT Detection & Parametric Fit Demonstration",
        fontsize=14,
        fontweight="bold",
        pad=15,
    )
    ...
    ax2.plot(
        t_grid, c1d_even, color="#9b59b6", label="Even (Symmetric) Consensus SNR", linewidth=1.8
    )
    ax2.plot(
        t_grid,
        c1d_odd,
        color="#f1c40f",
        label="Odd (Asymmetric) Consensus SNR",
        linewidth=1.2,
        alpha=0.7,
    )
    ax2.axhline(
        ...
        color="#e74c3c",
        linestyle="--",
        label="Default Detection Threshold (12.0σ)",
        linewidth=1.0,
    )
    ...
    if results["anomalies"]:
        ax2.scatter(
            ...
            edgecolor="#ffffff",
            linewidth=1.5,
            label=f"Detection Peak ({anom['snr']:.1f}σ)",
        )
