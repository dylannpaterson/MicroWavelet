...
def generate_detrend_plot():
    ...
    ax1.scatter(
        ...
        alpha=0.4,
        s=25,
        label="Clipped Transients",
        zorder=3,
    )
    ax1.errorbar(
        t[search_mask],
        y_raw[search_mask],
        yerr=y_err[search_mask],
        fmt="o",
        color="#7f8c8d",
        alpha=0.6,
        markersize=3,
    ...
    ax1.set_title(
        "Step 1: Robust Period Search (Masking Transients)",
        loc="left",
        fontsize=13,
        fontweight="bold",
    )
    ...
    ax2.scatter(
        ...
        alpha=0.5,
        s=10,
        label="Folded Baseline",
    )
    ax2.plot(
        phase[sort_idx],
        baseline_model[sort_idx],
        color="#e74c3c",
        linewidth=3,
        label=f"GP Model (P={recovered_period:.2f}d)",
        zorder=4,
    )
    ax2.set_ylabel("Relative Flux", fontweight="bold", fontsize=11)
    ax2.set_xlabel("Phase", fontweight="bold", fontsize=11)
    ax2.set_title(
        "Step 2: Iterative GP Baseline Modeling", loc="left", fontsize=13, fontweight="bold"
    )
    ...
    ax3.plot(
        t, A, color="#2ecc71", linewidth=2.5, label="True Microlensing Signal", alpha=0.9, zorder=3
    )
    ...
    plt.suptitle(
        "MicroWavelet: Periodic Baseline Detrending Pipeline",
        fontsize=18,
        fontweight="bold",
        y=0.98,
    )
