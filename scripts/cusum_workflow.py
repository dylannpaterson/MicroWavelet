...
def generate_microlensing_data(t, t0, u0, tE, baseline=1.0, anomaly_t=None, anomaly_amp=0.0, anomaly_width=None, noise_std=0.02):
...
def run_demo():
    ...
    anomaly_t = 60
    anomaly_amp = 0.3
    anomaly_width = 0.5  # Much narrower than 0.1 * tE (which is 1.5)
    ...
    # Panel 2: Event CUSUM (Residuals vs Baseline + Score)
    ax = axes[1]
    ax_res = ax.twinx()
    ax.plot(t, residuals_baseline, 'k.', alpha=0.2, label='Residuals (vs Baseline)')
    ax_res.plot(t, cusum_event['cusum_statistic'], 'g-', label='Event CUSUM Score')
    if cusum_event['triggered']:
        ax.axvspan(cusum_event['onset'], cusum_event['end'], color='green', alpha=0.1)
    ax.set_title("2. Event Detection (Residuals vs Baseline)")
    ax.set_ylabel("Residuals")
    ax_res.set_ylabel("CUSUM Score")
    ax.legend(loc='upper left')
    ax_res.legend(loc='upper right')

    # Panel 3: Anomaly CUSUM (Residuals vs PSPL + Score)
    ax = axes[2]
    ax_res = ax.twinx()
    ax.plot(t, residuals_pspl, 'k.', alpha=0.2, label='Residuals (vs PSPL)')
    ax_res.plot(t, cusum_anomaly['cusum_statistic'], 'b-', label='Anomaly CUSUM Score')
    if cusum_anomaly['triggered']:
        ax.axvspan(cusum_anomaly['onset'], cusum_anomaly['end'], color='blue', alpha=0.1)
    ax.set_title("3. Anomaly Detection (Residuals vs PSPL)")
    ax.set_ylabel("Residuals")
    ax_res.set_ylabel("CUSUM Score")
    ax.legend(loc='upper left')
    ax_res.legend(loc='upper right')

    # Panel 4: CUSUM Statistics Comparison (Score only)
    ax = axes[3]
    ...
    plt.savefig("docs/microlensing_cusum_workflow_v2.png", dpi=150)
    print("Saved docs/microlensing_cusum_workflow_v2.png")
