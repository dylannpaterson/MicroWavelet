...
def test_te_accuracy():
    ...
    print(
        f"{'u0_true':<10} | {'tE_true':<10} | {'tE_scan':<10} | {'tE_est':<10} | {'Error (%)':<10}"
    )
    print("-" * 70)
    ...
    for u0_true in u0_list:
        ...
        data = {"test_band": {"t": t_obs, "y": y_obs, "y_err": y_err}}
        ...
        if anomalies:
            ...
            print(
                f"{u0_true:<10.2f} | {tE_true:<10.1f} | {tE_scan:<10.2f} | {tE_est:<10.2f} | {error:<10.2f}"
            )
        else:
            print(
                f"{u0_true:<10.2f} | {tE_true:<10.1f} | {'FAILED':<10} | {'FAILED':<10} | {'N/A':<10}"
            )
