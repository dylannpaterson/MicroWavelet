...
def test_periodic_robustness_standard_case():
    ...
    assert abs(recovered_A - true_A) / true_A < 0.1, (
        f"Event signal suppressed. Found A_peak={recovered_A}, expected ~{true_A}"
    )
...
def test_cv_period_recovery():
    ...
    assert abs(recovered - period_true) < 0.1, (
        f"Failed to recover true CV period. Found {recovered}, expected {period_true}"
    )
