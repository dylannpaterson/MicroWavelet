...
def generate_microlensing_data(
    t, t0, u0, tE, baseline=1.0, anomaly_t=None, anomaly_amp=0.0, anomaly_width=None, noise_std=0.01
):
...
def run_demo():
    ...
    cusum_event = find_anomalies_cusum(
        t, residuals_baseline_sigma, threshold=25.0, k=1.0, bidirectional=True
    )
    ...
    cusum_anomaly = find_anomalies_cusum(
        t, residuals_pspl_sigma, threshold=12.0, k=2.0, bidirectional=True
    )
