...
def test_new_features():
    ...
    for a in res_linear["anomalies"]:
        print(
            f"t0: {a['t0']:.2f}, tE: {a['tE']:.2f}, snr: {a['snr']:.2f}, dchi2: {a['dchi2']:.2f}, dbic: {a['dbic']:.2f}, edge: {a['edge_flag']}"
        )
    ...
    assert len(res_filtered_none["anomalies"]) == 0, (
        "High min_dchi2 should filter out all anomalies"
    )
    ...
    data_achromatic = {
        "band1": {
            "t": t_multi,
            "y": y_flux1 + np.random.normal(0, 0.005, size=len(t_multi)),
            "y_err": np.ones_like(t_multi) * 0.005,
        },
        "band2": {
            "t": t_multi,
            "y": y_flux2 + np.random.normal(0, 0.005, size=len(t_multi)),
            "y_err": np.ones_like(t_multi) * 0.005,
        },
    }
    ...
    for a in res_achromatic["anomalies"]:
        print(
            f"t0: {a['t0']:.2f}, chromatic: {a['chromatic_flag']}, ratio: {a['chromaticity_ratio']:.3f}"
        )
    ...
    data_chromatic = {
        ...
        "band2": {
            "t": t_multi,
            "y": 1.0 + np.random.normal(0, 0.005, size=len(t_multi)),
            "y_err": np.ones_like(t_multi) * 0.005,
        },  # completely flat
    }
    ...
    print(
        f"Sigma White: {metrics_white['sigma_white']:.4f}, Total: {metrics_white['sigma_total']:.4f}"
    )
    print(
        f"Autocorr Lag 1: {metrics_white['autocorr_lag1']:.4f}, Has Red Noise: {metrics_white['has_red_noise']}"
    )
    ...
    print(
        f"Autocorr Lag 1: {metrics_red['autocorr_lag1']:.4f}, Has Red Noise: {metrics_red['has_red_noise']}"
    )
...
def test_bjd_scaling_recovery():
    ...
    print(
        f"Base t0: {t0_base:.6f}, Offset t0: {t0_offset:.6f} (Expected close to {t0_base + offset:.6f})"
    )
    print(f"Base SNR: {snr_base:.4f}, Offset SNR: {snr_offset:.4f}")

    # Assert unscaled coordinates are recovered perfectly
    assert abs(t0_offset - (t0_base + offset)) < 1e-8, (
        "Precision loss or incorrect offset recovery in peak t0"
    )
    assert np.isclose(snr_offset, snr_base, rtol=1e-3), (
        f"Numerical instability affected peak SNR: {snr_offset} vs {snr_base}"
    )
