...
def detect_anomalies_with_fit(
    t, y, y_err, threshold=25.0, k=2.0, threshold_slow=25.0, k_slow=0.5, bidirectional=True
):
    """
    ...
    seeds_fast = seed_by_flat_cusum(
        t, y, y_err, method="linear", k=2.0, threshold=10.0, return_all=True
    )
    seeds_slow = seed_by_flat_cusum(
        t, y, y_err, method="linear", k=0.5, threshold=10.0, return_all=True
    )
    ...
    for t0_seed, tE_seed in seeds:
        ...
        bounds = [(t0_seed - half_width, t0_seed + half_width), (-3.0, 1.0), (0.0, 3.0)]
    ...
    anom_fast = find_anomalies_cusum(
        t, residuals_sigma, threshold=threshold, k=k, bidirectional=bidirectional
    )
    anom_slow = find_anomalies_cusum(
        t, residuals_sigma, threshold=threshold_slow, k=k_slow, bidirectional=bidirectional
    )
